...
public class SearchRelatedResultsDocumentDisplayContextBuilder {
	...
	protected void buildImage(
		...
		if (assetClassName.equals(className)) {
			...
			try {
				...
				if (Validator.isNull(thumbnailURLString)) {
					thumbnailURLString = blogsEntry.getSmallImageURL(_themeDisplay);
	            }
				...
				if (_log.isWarnEnabled()) {
					_log.warn(
						StringBundler.concat(
								"Blogs entry thumnamil url exception ",
								"and contains blogs entry id ", blogsEntry.getEntryId()));
				}
			}
        	searchRelatedResultsDocumentDisplayContext.setThumbnailURLString(
        		thumbnailURLString);  
		...
		if (assetClassName.equals(className)) {		
			try {
				...
				if (_log.isWarnEnabled()) {
					_log.warn(
						StringBundler.concat(
							"JournalArticle entry thumnamil url exception ",
							"and contains journal article classPK ", classPK));
				}
			}
			return;
		}
		...
		if (assetClassName.equals(className)) {
			...
			if (assetObject instanceof FileEntry) {
				...
				searchRelatedResultsDocumentDisplayContext.setIconId(
					fileEntry.getIcon());
				
				try {
					thumbnailURLString = DLURLHelperUtil.getThumbnailSrc(fileEntry, _themeDisplay);
				} catch (Exception e) {
					if (_log.isWarnEnabled()) {
						_log.warn(
							StringBundler.concat(
								"File entry thumnamil url exception ",
								"and contains file classPK ", classPK));
					}
				}
				searchRelatedResultsDocumentDisplayContext.setThumbnailURLString(thumbnailURLString);				
			}
			else {
	...
	private static final Log _log = LogFactoryUtil.getLog(
		SearchRelatedResultsDocumentDisplayContextBuilder.class);
