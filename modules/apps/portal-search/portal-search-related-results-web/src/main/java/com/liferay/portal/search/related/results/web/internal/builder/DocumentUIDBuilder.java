...
public class DocumentUIDBuilder {
	...
	public static final String ENTRYCLASSNAME = "entryClassName";
	...
	protected String getUIDFromURL(
		...
		AssetEntryDocumentUIDBuilder assetEntryDocumentUIDBuilder = null;
		...
		if (ASSET_PUBLISHER.equals(modelURLParameters[0]) &&
			...
			assetEntryDocumentUIDBuilder =
				new AssetPublisherDocumentUIDBuilder().setGroupId(
					_groupId
				).setId(
					Long.valueOf(parameterValues[2])
				);

			uid = assetEntryDocumentUIDBuilder.build();
		}
			...
			assetEntryDocumentUIDBuilder =
				new BlogsEntryDocumentUIDBuilder().setGroupId(
					_groupId
				).setUrlTitle(
					parameterValues[1]
				);
			...
			assetEntryDocumentUIDBuilder =
				new MBMessageDocumentUIDBuilder().setGroupId(
					_groupId
				).setId(
					Long.valueOf(parameterValues[2])
				).setMBType(
			...
			assetEntryDocumentUIDBuilder =
				new DLFileEntryDocumentUIDBuilder().setGroupId(
					_groupId
				).setId(
		...
		if (!Validator.isBlank(uid)) {
			setEntryClassName(
				assetEntryDocumentUIDBuilder.getAssetEntryClassName());
	...
	protected String getWikiDiaplayUIDFromURL(
		...
		WikiPageDocumentUIDBuilder builder =
			new WikiPageDocumentUIDBuilder().setGroupId(
				_groupId
			).setNodeName(
				nodeName
			).setTitle(
		...
		String uid = builder.build();
		setEntryClassName(builder.getAssetEntryClassName());

		return uid;
	}

	protected void setEntryClassName(String entryClassName) {
		_renderRequest.setAttribute(ENTRYCLASSNAME, entryClassName);
	}
