...
public class SXPBlueprintIndexConfigurationTest {
	...
	public void testSXPBlueprintIndexSearch() throws Exception {
		...
		_assertSearch("[" + title1 + "]", title1, sxpBlueprint);

		addSynonymSet(title1 + "," + title2);

		_assertSearch(
			StringBundler.concat("[", title1, ", ", title2, "]"), title1,
			sxpBlueprint);

		sxpBlueprint = _addSXPBlueprint(
			_user.getUserId(), _getSynomySetIndexName());

		_assertSearch(null, title1, sxpBlueprint);
	}
	...
	private void _assertSearch(
			String expected, String keywords, SXPBlueprint sxpBlueprint)
		throws Exception {
