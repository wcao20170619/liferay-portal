...
public class SXPBlueprintSearchResultTestUtil {
	...
	public static String getElementInstancesJSON(
		String[] sxpElementNames, Object[] configurationValuesArray,
		...
		for (int i = 0; i < sxpElementNames.length; i++) {
			...
			com.liferay.search.experiences.rest.dto.v1_0.SXPElement
				sxpElementDTO = SXPElementUtil.toSXPElement("{}");

			sxpElementDTO.setElementDefinition(elementDefinition);

			elementInstance.setSxpElement(sxpElementDTO);

			elementInstance.setUiConfigurationValues(
				_createUiConfigurationValues(
	...
	private static Map<String, Object> _createUiConfigurationValues(
		UiConfiguration uiConfiguration, Map<String, Object> values) {

		if ((uiConfiguration == null) ||
			(uiConfiguration.getFieldSets() == null) || (values == null)) {
		...
		for (FieldSet fieldSet : fieldSets) {
			...
			for (Field field : fields) {
				...
				uiConfigurationValues.put(fieldName, UnpackUtil.unpack(value));
			}
		}
